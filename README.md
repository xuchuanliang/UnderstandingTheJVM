...
# 第八章 虚拟机字节码执行引擎
>java编译器：读取java源文件并编译成java字节码文件
 java解释器：java编译器生成的class文件不能直接被硬件中的CPU直接执行，java解释器就是能够解释并执行字节码文件，就像根植于软件中的CPU
 JIT编译器（即时编译器）：java程序最初是通过解释器进行进行解释执行的，当虚拟机发现某个方法或代码块运行的特别频繁时，会把这些代码块
 认定为热点代码，为了提高热点代码的执行效率，在运行时，虚拟机会把这些代码编译成本地机器码，并进行各种层次的优化，完成这个任务的编译器称为JIT编译器。
 其实说白了JIT编译器就是将字节码编译成机器码
 解释器+JIT编译器就是JVM执行引擎
